...
class RegScorer(Scorer):
    ...
    def get_results(self):
        ...
        rmse_std = np.std(np.sqrt(self.mses))
...
class ClassScorer(Scorer):
    ...
    def eval(self, y_true, y_prediction, y_probabilities):
        y_probabilities = nn.Softmax(dim=1)(torch.tensor(y_probabilities,dtype=float)).numpy()
...
if __name__ == "__main__":
    sc = ClassScorer()
    sc.eval([0,1,2,1], [0,1,2,1],[[2,1,1],[1,3,1],[0,1,3],[2,10,0]]) 
