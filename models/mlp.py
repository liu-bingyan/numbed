...
class MLP(BaseModelTorch):
    ...
    def __init__(self, params, args):
        ...
        self.model = MLP_Model(n_layers=self.params["n_layers"], input_dim=self.adjusted_input_dim,
...
class MLP_Model(nn.Module):
    ...
    def __init__(self, n_layers, input_dim, hidden_dim, output_dim, task):
        ...
        if isinstance(hidden_dim, int):
            ...
            # Hidden Layers (number specified by n_layers)
            self.layers.extend([nn.Linear(hidden_dim[i], hidden_dim[i+1]) for i in range(len(hidden_dim)-1)])
