...
class TabTransformer(BaseModelTorch):
    ...
    def predict_helper(self, X):
        ...
        #test_loader = DataLoader(dataset=test_dataset, batch_size=self.args.val_batch_size, shuffle=False, num_workers=2)
        test_dataset = TensorDataset(X)
        if self.args.data_loader == "torch":
...
if __name__ == "__main__":
    ...
    args = Namespace(dataset="CaliforniaHousing",
                     small_sample=False,
                     target_encode=True,
                     scale=True,
                     scale_target=True,
    ...
    from numerical_embedding.utils.load_data import load_data
    X,y = load_data(args)
